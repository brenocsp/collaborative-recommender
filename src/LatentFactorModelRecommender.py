...
class LatentFactorModelRecommender:
    ...
    def runRecommender(self, training, validation, targets, startTime, saveToFile=True, printOnConsole=True):
        ...
        if saveToFile:
            submissionFile = pd.DataFrame(self.predictions_, columns=['UserId:ItemId', 'Rating'])
            submissionFile.to_csv('submission.csv', index=False, sep=',')
        
        if printOnConsole:
            print('UserId:ItemId,Rating')
            for userItem, rating in self.predictions_:
                print(userItem, rating, sep=',')